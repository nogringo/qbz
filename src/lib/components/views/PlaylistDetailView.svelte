<script lang="ts">
  import { ArrowLeft, Play, Shuffle, ListMusic, Search, X, ChevronDown, ChevronRight, ImagePlus, HardDrive, Info, Edit3, BarChart2, Sparkles, Plus, RefreshCw, Loader2 } from 'lucide-svelte';
  ...
  let suggestedTracks = $state<DisplayTrack[]>([]);
  let loadingSuggestions = $state(false);
  let suggestionsExpanded = $state(false);
  let dismissedTrackIds = $state<Set<number>>(new Set());

  // Load dismissed tracks from localStorage on init
  function loadDismissedTracks() {
    try {
      const stored = localStorage.getItem(`playlist_${playlistId}_dismissed`);
      if (stored) {
        dismissedTrackIds = new Set(JSON.parse(stored));
      } else {
  ...
  function saveDismissedTracks() {
    localStorage.setItem(`playlist_${playlistId}_dismissed`, JSON.stringify([...dismissedTrackIds]));
  }
  ...
  $effect(() => {
    ...
    loadStats();
    loadDismissedTracks();
    // Reset suggestions when changing playlists
  ...
  async function loadSuggestions() {
    if (loadingSuggestions || tracks.length === 0) return;

    loadingSuggestions = true;
    suggestedTracks = [];

    try {
      // Get track IDs and artist IDs from playlist
      const playlistTrackIds = tracks.map(t => t.id);
      const playlistArtistIds = tracks
        .filter(t => t.artistId)
        .map(t => t.artistId as number);

      // Get suggestions from ML system
      const suggestionIds = await invoke<number[]>('get_playlist_suggestions', {
        playlistTrackIds,
        playlistArtistIds,
        limit: 8
      });
      ...
      const trackPromises = suggestionIds.map(async (trackId) => {
        try {
      ...
      suggestedTracks = results
        ...
        .filter(t => !dismissedTrackIds.has(t.id));
    } catch (err) {
  ...
  function dismissSuggestion(track: DisplayTrack) {
    // Add to dismissed set
    dismissedTrackIds.add(track.id);
    dismissedTrackIds = new Set(dismissedTrackIds); // Trigger reactivity
    saveDismissedTracks();
    // Remove from current suggestions
    suggestedTracks = suggestedTracks.filter(t => t.id !== track.id);
  }

  async function addSuggestionToPlaylist(track: DisplayTrack) {
    try {
      ...
      <div class="suggestions-section">
        <button
          ...
          onclick={() => {
            ...
            if (suggestionsExpanded && suggestedTracks.length === 0) {
              loadSuggestions();
            }
          }}
        >
          ...
          <div class="suggestions-actions">
            {#if suggestionsExpanded && suggestedTracks.length > 0}
              ...
              >
                <RefreshCw size={14} class={loadingSuggestions ? 'spinning' : ''} />
              </button>
            {/if}
          ...
          <div class="suggestions-content">
            {#if loadingSuggestions}
              <div class="suggestions-loading">
                ...
                <span>Finding tracks you might like...</span>
              </div>
            {:else if suggestedTracks.length === 0}
              <div class="suggestions-empty">
                <p>No suggestions available yet. Play more music to get personalized recommendations!</p>
              </div>
            {:else}
              <div class="suggestions-list">
                {#each suggestedTracks as track (track.id)}
                  <div class="suggestion-row">
                    ...
                    <div class="suggestion-info">
                      <span class="suggestion-title">{track.title}</span>
                      <span class="suggestion-artist">{track.artist || 'Unknown Artist'}</span>
                    </div>
                    <span class="suggestion-duration">{track.duration}</span>
                    <button
                      class="dismiss-suggestion-btn"
                      onclick={() => dismissSuggestion(track)}
                    ...
                    <button
                      ...
                      onclick={() => addSuggestionToPlaylist(track)}
...
<style>
  ...
  .suggestion-art .art-placeholder {
    width: 100%;
    height: 100%;
    display: flex;
    align-items: center;
